...
const PROMPTS = {
  lang: (d) => `당신은 LEET 언어이해 문제 출제 전문가입니다. 난이도 ${d}/5 문제를 1개 만들어주세요. 난이도가 높을수록 지문이 길고 복잡하며 함정 선지가 많습니다. 반드시 아래 JSON만 출력하세요(마크다운 없이):\n{"type":"독해|추론|빈칸추론|강화약화 중 하나","difficulty":${d},"passage":"지문(법철학·계약법·형사법·헌법 관련)","question":"질문","options":["선지1","선지2","선지3","선지4","선지5"],"answer":정답인덱스0~4,"explanation":"해설"}`,
  reason: (d) => `당신은 LEET 추리논증 문제 출제 전문가입니다. 난이도 ${d}/5 문제를 1개 만들어주세요. 유형: 형식논리, 논증강화, 법적추론, 퍼즐추리 중 하나. 반드시 아래 JSON만 출력하세요(마크다운 없이):\n{"type":"형식논리|논증강화|법적추론|퍼즐추리 중 하나","difficulty":${d},"passage":"조건/지문","question":"질문","options":["선지1","선지2","선지3","선지4","선지5"],"answer":정답인덱스0~4,"explanation":"단계별 해설"}`,
  strategy: (d) => `당신은 LEET 학습전략 전문가입니다. 난이도 ${d}/5 전략 퀴즈를 1개 만들어주세요. 반드시 아래 JSON만 출력하세요(마크다운 없이):\n{"type":"시간전략|오답분석|기출분석|모의고사 중 하나","difficulty":${d},"passage":null,"question":"질문","options":["선지1","선지2","선지3","선지4","선지5"],"answer":정답인덱스0~4,"explanation":"해설"}`
};
...
async function generateQuestion(tab, difficulty) {
  ...
  if (!res.ok) {
    ...
    throw new Error("서버 오류 " + res.status + ": " + errText.slice(0, 300));
  }
    ...
    <div style={{ maxWidth: 700, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18, padding: "10px 16px", background: "#0F0F1A", borderRadius: 12, border: "1px solid #1E2030" }}>
        ...
        <div style={{ flex: 1, height: 4, background: "#1E293B", borderRadius: 4, overflow: "hidden" }}>
          {sessionStats.total > 0 && <div style={{ height: "100%", width: `${Math.round(sessionStats.correct / sessionStats.total * 100)}%`, background: "linear-gradient(90deg,#34D399,#2DD4BF)", borderRadius: 4 }} />}
        </div>
      ...
      {q.passage && (
        <div style={{ background: "#12121F", border: "1px solid #1E2030", borderLeft: `3px solid ${color}`, borderRadius: "0 12px 12px 0", padding: "16px 20px", marginBottom: 18, fontSize: 13.5, color: "#CBD5E1", lineHeight: 1.95, whiteSpace: "pre-line" }}>{q.passage}</div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 9, marginBottom: 20 }}>
        {q.options.map((opt, i) => {
          ...
          } else if (selected === i) { bg = color + "11"; border = color; textColor = "#F1F5F9"; }
          return (
      ...
      {selected !== null && !revealed && (
        <button onClick={() => setRevealed(true)} style={{ width: "100%", padding: 13, borderRadius: 10, marginBottom: 16, background: `linear-gradient(135deg,${color},${color}99)`, border: "none", color: "#000", fontWeight: 800, fontSize: 14, cursor: "pointer" }}>정답 확인 →</button>
      )}
      {revealed && (
        <div style={{ background: correct ? "#0D2B1E" : "#2B0D0D", border: `1px solid ${correct ? "#34D399" : "#EF4444"}55`, borderRadius: 12, padding: "16px 20px", marginBottom: 20 }}>
          <div style={{ fontSize: 14, fontWeight: 800, color: correct ? "#34D399" : "#EF4444", marginBottom: 8 }}>{correct ? "✅ 정답!" : "❌ 오답"}</div>
          <div style={{ fontSize: 13, color: "#94A3B8", lineHeight: 1.85 }}><span style={{ fontWeight: 700, color: "#CBD5E1" }}>해설 </span>{q.explanation}</div>
        </div>
      )}
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={() => { onPrev(); reset(); }} disabled={idx === 0} style={{ flex: 1, padding: 11, borderRadius: 10, border: "1px solid #1E2030", background: "#12121F", color: idx === 0 ? "#334155" : "#94A3B8", cursor: idx === 0 ? "not-allowed" : "pointer", fontWeight: 600, fontSize: 13 }}>← 이전</button>
        <button onClick={reset} style={{ padding: "11px 14px", borderRadius: 10, border: "1px solid #1E2030", background: "#12121F", color: "#64748B", cursor: "pointer", fontSize: 12 }}>↺</button>
        {idx < total - 1 ? (
          <button onClick={() => { onNext(); reset(); }} style={{ flex: 1, padding: 11, borderRadius: 10, border: `1px solid ${color}44`, background: color + "22", color, cursor: "pointer", fontWeight: 600, fontSize: 13 }}>다음 →</button>
        ) : (
          <button onClick={onNewAI} disabled={isGenerating} style={{ flex: 2, padding: 11, borderRadius: 10, border: `1px solid ${color}66`, background: isGenerating ? "#1E293B" : `linear-gradient(135deg,${color}33,${color}11)`, color: isGenerating ? "#475569" : color, cursor: isGenerating ? "not-allowed" : "pointer", fontWeight: 800, fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
...
export default function LEETMap() {
  ...
  const getNextDifficulty = useCallback(() => Math.min(5, Math.max(2, Math.ceil(sessionStats.correct / 2) + 2)), [sessionStats.correct]);
  ...
  const handleGenerateAI = useCallback(async (tabOverride) => {
    ...
    setGenError(null);
    try {
      const difficulty = Math.min(5, Math.max(2, Math.ceil(sessionStats.correct / 2) + 2));
  ...
  }, [quizTab, sessionStats.correct]);

  useEffect(() => {
    if (view === "quiz" && !autoLoaded[quizTab] && questions[quizTab].length === 0 && !isGenerating) {
      setAutoLoaded(p => ({ ...p, [quizTab]: true }));
      handleGenerateAI(quizTab);
    }
  }, [view, quizTab]);

  const handleReveal = useCallback((questionId, isCorrect) => {
    if (answeredSet.has(questionId)) return;
    setAnsweredSet(prev => new Set([...prev, questionId]));
    setSessionStats(prev => ({ correct: prev.correct + (isCorrect ? 1 : 0), wrong: prev.wrong + (isCorrect ? 0 : 1), total: prev.total + 1 }));
  }, [answeredSet]);
          ...
          <>
            <div style={{ width: 56, background: "#0F0F1A", borderRight: "1px solid #1E2030", display: "flex", flexDirection: "column", alignItems: "center", padding: "12px 0", gap: 7 }}>
              {LEET_DATA.sections.map(s => (
                <button key={s.id} onClick={() => { setActiveSection(s.id); setActiveTopic(null); }} title={s.title} style={{ width: 40, height: 40, borderRadius: 11, background: activeSection === s.id ? s.bg : "transparent", border: activeSection === s.id ? `2px solid ${s.color}` : "2px solid transparent", fontSize: 19, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>{s.icon}</button>
              ))}
            </div>
            <div style={{ width: 260, background: "#0C0C18", borderRight: "1px solid #1E2030", display: "flex", flexDirection: "column", overflow: "hidden" }}>
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: 9 }}>
                {section.topics.map(topic => {
                  ...
                  return (
                    <button key={topic.id} onClick={() => setActiveTopic(isActive ? null : topic)} style={{ width: "100%", textAlign: "left", background: isActive ? section.bg : "#12121F", border: isActive ? `1px solid ${section.color}44` : "1px solid #1E2030", borderRadius: 11, padding: "10px 12px", marginBottom: 6, cursor: "pointer", transition: "all 0.2s" }}>
                      ...
                      <div style={{ fontSize: 9, color: "#334155", marginTop: 4 }}><span style={{ color: "#F59E0B" }}>{DIFF_LABEL[topic.difficulty]}</span> · {topic.time}</div>
                    </button>
                  );
                })}
              </div>
              ...
              {!activeTopic ? (
                <div style={{ height: "100%", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
                  ...
                  <div style={{ position: "relative", width: 460, height: 290 }}>
                    ...
                    {[{ label: "언어이해", x: 170, y: 52, color: "#2DD4BF", size: 58, icon: "📖" }, { label: "추리논증", x: 240, y: 190, color: "#F59E0B", size: 58, icon: "⚖️" }, { label: "독해 기반", x: 52, y: 118, color: "#2DD4BF88", size: 40 }, { label: "추론·응용", x: 70, y: 193, color: "#2DD4BF88", size: 40 }, { label: "형식논리", x: 355, y: 93, color: "#F59E0B88", size: 40 }, { label: "논증 분석", x: 372, y: 192, color: "#F59E0B88", size: 40 }, { label: "법적 추론", x: 267, y: 258, color: "#8B5CF688", size: 40 }].map((n, i) => (
                  ...
                  <div style={{ marginTop: 16, fontSize: 11, color: "#475569", textAlign: "center" }}>언어이해 <span style={{ color: "#2DD4BF" }}>추론·응용</span>↔ 추리논증 <span style={{ color: "#F59E0B" }}>논증 분석</span> 스킬 공유</div>
                </div>
              ) : (
                  ...
                  <div style={{ marginBottom: 18 }}>
                    ...
                    <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                      {activeTopic.linked.map(lid => {
                        ...
                        return <button key={lid} onClick={() => { const sec = LEET_DATA.sections.find(s => s.topics.some(t => t.id === lid)); if (sec) { setActiveSection(sec.id); setActiveTopic(sec.topics.find(t => t.id === lid)); } }} style={{ background: linked.sectionColor + "11", border: `1px solid ${linked.sectionColor}44`, borderRadius: 18, padding: "4px 11px", color: linked.sectionColor, fontSize: 11, fontWeight: 600, cursor: "pointer" }}>→ {linked.title}</button>;
                      })}
                    </div>
                  </div>
                  <div style={{ background: "#0F0F1A", border: `1px solid ${section.color}33`, borderRadius: 11, padding: "14px 17px" }}>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {[0, 20, 40, 60, 80, 100].map(val => (
                        <button key={val} onClick={() => setMastery(activeTopic.id, val)} style={{ padding: "6px 12px", borderRadius: 18, fontSize: 11, fontWeight: 700, cursor: "pointer", background: getMastery(activeTopic.id) === val ? section.color : "#1E293B", color: getMastery(activeTopic.id) === val ? "#000" : "#64748B", border: getMastery(activeTopic.id) === val ? `1px solid ${section.color}` : "1px solid #334155" }}>
        ...
        {view === "quiz" && (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ background: "#0F0F1A", borderBottom: "1px solid #1E2030", padding: "9px 22px", display: "flex", gap: 7, alignItems: "center" }}>
              {QUIZ_TABS.map(t => (
                <button key={t.id} onClick={() => setQuizTab(t.id)} style={{ background: quizTab === t.id ? t.color + "22" : "transparent", border: quizTab === t.id ? `1px solid ${t.color}55` : "1px solid transparent", borderRadius: 18, padding: "5px 15px", color: quizTab === t.id ? t.color : "#64748B", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>{t.label}</button>
              ))}
              <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ fontSize: 11, color: "#475569" }}>다음 AI 난이도: <span style={{ color: "#F59E0B", fontWeight: 800 }}>{DIFF_LABEL[getNextDifficulty()]}</span></div>
                <div style={{ display: "flex", gap: 3 }}>
                  {curQList.map((_, i) => (
                    <div key={i} onClick={() => setQuizIdx(p => ({ ...p, [quizTab]: i }))} style={{ width: 7, height: 7, borderRadius: "50%", background: i === curQIdx ? quizColor : curQList[i]?.isAI ? "#818CF844" : "#1E293B", border: `1px solid ${i === curQIdx ? quizColor : "#334155"}`, cursor: "pointer" }} />
                  ))}
                </div>
              </div>
            </div>
            {genError && <div style={{ background: "#2B0D0D", border: "1px solid #EF444444", borderRadius: 9, padding: "10px 16px", margin: "12px 22px 0", fontSize: 12, color: "#EF4444" }}>⚠️ {genError}</div>}
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 26 }}>
            <div style={{ maxWidth: 640, margin: "0 auto" }}>
              ...
              <div style={{ background: "#0F0F1A", border: "1px solid #818CF833", borderRadius: 13, padding: "18px 20px", marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", gap: 16 }}>
                  {[{ label: "푼 문제", val: sessionStats.total, color: "#CBD5E1" }, { label: "정답", val: sessionStats.correct, color: "#34D399" }, { label: "오답", val: sessionStats.wrong, color: "#EF4444" }, { label: "정답률", val: sessionStats.total > 0 ? `${Math.round(sessionStats.correct / sessionStats.total * 100)}%` : "-", color: "#F59E0B" }].map(s => (
                    <div key={s.label} style={{ flex: 1, background: "#12121F", border: "1px solid #1E2030", borderRadius: 10, padding: 12, textAlign: "center" }}>
                ...
                return (
                  <div key={s.id} style={{ background: "#0F0F1A", border: `1px solid ${s.color}22`, borderRadius: 13, padding: "19px 21px", marginBottom: 16 }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 13 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 7 }}><span style={{ fontSize: 17 }}>{s.icon}</span><span style={{ fontSize: 14, fontWeight: 800, color: s.color }}>{s.title}</span></div>
              ...
              <div style={{ background: avgMastery >= 70 ? "#0D2B29" : avgMastery >= 40 ? "#1A1A2E" : "#2B0D0D", border: `1px solid ${avgMastery >= 70 ? "#2DD4BF" : avgMastery >= 40 ? "#818CF8" : "#EF4444"}44`, borderRadius: 13, padding: "17px 20px", textAlign: "center" }}>
                ...
                <div style={{ fontSize: 13, color: "#94A3B8" }}>{avgMastery >= 80 ? "🏆 완성! 실전 모의고사 집중" : avgMastery >= 60 ? "💪 고급! 약점 파트 집중 공략" : avgMastery >= 40 ? "📚 중급! 꾸준히 유지" : avgMastery >= 20 ? "🌱 기초! 기출 분석부터" : "✨ 개념 맵에서 숙련도를 기록하세요"}</div>
              </div>
            </div>
          </div>
        )}
